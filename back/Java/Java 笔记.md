...
```
守护线程：当所有非守护线程全部执行完毕之后，守护线程不论是否执行完毕，都会终止。而非守护线程独立于主线程运行，也就是说即便主线程结束，而只要有一个非守护线程在执行，那么Java虚拟机就不会退出，除非所有的非守护线程就运行结束。
```
...
（3）锁对象
	...
	·· 普通同步方法：同步方法所在类的实例（this）。由于一个类的示例可以有多个，所以要保证线程安全的话，那么只有使用唯一的一个示例在不同的线程中去调用同步方法才可以，如果有多个示例在多个线程中去调用这个同步方法，那么就无法保证线程安全，因为使用了多个锁对象
	·· 静态同步方法：静态同步方法所在类的class对象。由于任意一个类只有唯一的一个class对象与之对应，就是类加载器加载完class文件到内存之后，内存中为该类创建的一个class对象。所以该类的任意一个示例在多个线程中调用静态同步方法，都可以保证线程的安全性
```
...
（3）使用：
	...
	//锁住的代码块
	...
	//解锁（注意解锁代码一般放在finally中，确保即便程序出现问题也可以顺利解锁）
	lock.unlock();
```
